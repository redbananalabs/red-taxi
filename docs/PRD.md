...
| Webhook secret (dev) | `whsec_*` | Auto-generated by Stripe CLI `stripe listen` |
| Webhook secret (prod) | `whsec_*` | Environment variables (created in Stripe Dashboard) |
...
# Login (one-time, opens browser for auth)
stripe login

# Forward webhooks to local API (run alongside the app)
stripe listen --forward-to https://localhost:5001/api/stripe/webhook

# Output: whsec_... → put this in appsettings.Development.json
```

No manual webhook endpoint creation needed in Stripe Dashboard for dev. Stripe CLI intercepts all test-mode events and forwards them to localhost.
...
| Stripe CLI | Webhook forwarding + product seeding | `winget install Stripe.StripeCLI` then `stripe login` |
