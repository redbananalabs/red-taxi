...
namespace RedTaxi.Services
{
    public class TariffService : BaseService<TariffService>
    {
        ...
        public TariffService(
            RedTaxiDbContext dB,ILogger<TariffService> logger)
            : base(dB, logger)
        {
        ...
        private const string ApiKey = "AIzaSyBKnZQuHuCpXEL-90ZhQsSElExHQHrZOog";

        public async Task<JourneyDetails> GetDrivingDistance(string originPostcode, string destinationPostcode)
        {
            string requestUrl = $"https://maps.googleapis.com/maps/api/distancematrix/json?origins={originPostcode}&destinations={destinationPostcode}&key={ApiKey}";
