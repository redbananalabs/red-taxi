...
using AceTaxis.Domain.IdealPostcodes;
...
namespace AceTaxis.Services
{
    public class AddressLookupService : BaseService<AddressLookupService>, IAddressLookupService
    {
        ...
        public async Task<List<DTOs.Address.AddressSuggestion>> IdealPostcodeSearch(string postcode)
        {
            var data = await _idealPostcodes.LookupPostcodeAsync(postcode);
            ...
            foreach (var item in data)
            {
                ...
                if (!string.IsNullOrEmpty(item.OrganisationName))
                    arr.Add(item.OrganisationName.Trim());
                if (!string.IsNullOrEmpty(item.BuildingName))   
                    arr.Add(item.BuildingName.Trim());
                if (!string.IsNullOrEmpty(item.Thoroughfare))
                {
                    if (!string.IsNullOrWhiteSpace(item.BuildingNumber))
                        arr.Add($"{item.BuildingNumber} {item.Thoroughfare}".Trim());
                    else if (!string.IsNullOrEmpty(item.Premise))
                    {
                        arr.Add(item.Premise.Trim());
                        arr.Add(item.Thoroughfare.Trim());
                    }
                    else
                        arr.Add(item.Thoroughfare.Trim());
                }
                if (!string.IsNullOrEmpty(item.DependantLocality))
                    arr.Add(item.DependantLocality.Trim());
                if (!string.IsNullOrEmpty(item.PostTown))
                    arr.Add(item.PostTown.Trim());
                //if (!string.IsNullOrEmpty(item.Postcode))
                //    arr.Add(item.Postcode.Trim());

                 var addressLine = string.Join(", ", arr);



                lst.Add(new DTOs.Address.AddressSuggestion(
                      Id: $"",
                      Label: addressLine,
                      Type: "",
                      SecondaryText: null,
        ...
        { 
            ...
            var displayLabel = BuildDisplayLabel(data.BuildingName, data.BuildingNumber + " " + data.Thoroughfare,data.DependantLocality ,data.PostTown
                , data.Postcode, "");


            if(!string.IsNullOrEmpty(data.OrganisationName))
                displayLabel = data.OrganisationName.Trim() + ", " + displayLabel.TrimStart();
        ...
        private static string BuildDisplayLabel(
            string? placeName, string? line1, string locality,string? town, string? postcode, string? formattedFallback)
        {
