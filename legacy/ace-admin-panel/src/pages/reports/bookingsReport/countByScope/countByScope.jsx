...
import {
	refreshBookingScopeBreakdown,
	setBookingScopeBreakdown,
} from '../../../../slices/reportSlice';

function CountByScope() {
	const dispatch = useDispatch();
	const { bookingScopeBreakdown } = useSelector((state) => state.reports);
	...
	console.log(bookingScopeBreakdown);
	useEffect(() => {
		if (openDate) {
		...
		let baseColumns = [
			{
				...
				header: ({ column }) => (
					<DataGridColumnHeader
						title=<span className='font-bold'>Period Label</span>
			...
			{
				accessorKey: 'isComparison',
				header: ({ column }) => (
					<DataGridColumnHeader
						title=<span className='font-bold'>Comparison</span>
						column={column}
					/>
				),
				enableSorting: true,
					...
					<>
						<label className='switch switch-sm flex-1 sm:flex-none'>
							<input
								type='checkbox'
								name='check'
								checked={row.original.isComparison} // Controlled value
								readOnly
							/>
						</label>
					</>
	...
	const filteredBookings = useMemo(() => {
		return bookingScopeBreakdown.filter((booking) => {
			const search = searchInput.toLowerCase();
			return (
				booking.count?.toString().includes(search) || // Search by Booking ID
				booking.scopeText?.toLowerCase().includes(search) || // Search by Pickup Address
				booking.periodLabel?.toLowerCase().includes(search) // Search by Destination
			);
		});
	}, [bookingScopeBreakdown, searchInput]);

	useEffect(() => {
		return () => {
			dispatch(setBookingScopeBreakdown([])); // Clear table data
		};
	}, [dispatch]);
			...
			<div className='pe-[1.875rem] ps-[1.875rem] ms-auto me-auto max-w-[1850px] w-full'>
				<Toolbar>
					<ToolbarHeading>
						<ToolbarPageTitle />
						<ToolbarDescription>
							Showing {bookingScopeBreakdown?.length} Report(s){' '}
						</ToolbarDescription>
					</ToolbarHeading>
				</Toolbar>
			</div>
										...
										<div className='flex items-center gap-2'>
											<label className='switch switch-sm flex-1 sm:flex-none mt-4'>
												...
												<input
													type='checkbox'
													name='check'
													checked={compare} // Controlled value
													onChange={(e) => setCompare(e.target.checked)} // Update state on change
												/>
											</label>
										</div>
