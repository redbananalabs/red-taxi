...
import {
	refreshPayoutsByMonth,
	setPayoutsByMonth,
} from '../../../../slices/reportSlice';

function PayoutsByMonth() {
	const dispatch = useDispatch();
	const { payoutsByMonth } = useSelector((state) => state.reports);
	...
	const columns = useMemo(() => {
		let baseColumns = [
			...
			{
				accessorKey: 'totalPaymentDue',
				header: ({ column }) => (
					<DataGridColumnHeader
						title=<span className='font-bold'>Payment Due</span>
						filter={<ColumnInputFilter column={column} />}
						column={column}
					/>
				),
				...
				cell: ({ row }) => (
					<span className={`font-medium ${row.original.color}`}>
						£{row.original.totalPaymentDue.toFixed(2)}
					</span>
				),
	...
	useEffect(() => {
		return () => {
			dispatch(setPayoutsByMonth([])); // Clear table data
		};
	}, [dispatch]);
	return (
		<Fragment>
			<div className='pe-[1.875rem] ps-[1.875rem] ms-auto me-auto max-w-[1850px] w-full'>
				<Toolbar>
					<ToolbarHeading>
						<ToolbarPageTitle />
						<ToolbarDescription>
							Showing {payoutsByMonth?.length} Report(s){' '}
						</ToolbarDescription>
					</ToolbarHeading>
				</Toolbar>
			</div>
